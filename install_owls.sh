...
#------------------------------------------------------------------------------
# PATH to libraries
MYPATH_INTEL="$HOME/commander/cmake_update/build_owl3135/install"
MYPATH_GNU="$HOME/commander/quiet/build_owl3135/install"
...
then
  ...
  if [[ "${HOSTNAME}" =~ $owl_1724 ]]; then
	...
	abs_path_to_build="$camb_root_dir/$build_dir"
	...
	then
		...
		cmake --build $camb_root_dir/$build_dir --target install -j $physicalCpuCount 
	else
		...
		#cmake -DCMAKE_INSTALL_PREFIX:PATH="$camb_root_dir/$build_dir/install" -DCMAKE_BUILD_TYPE=Release -DCMAKE_Fortran_COMPILER=ifort -DCMAKE_C_COMPILER=icc -DCMAKE_CXX_COMPILER=icpc -DMPI_C_COMPILER=mpiicc -DMPI_CXX_COMPILER=mpiicpc -DMPI_Fortran_COMPILER=mpiifort -DCFITSIO_INCLUDE_DIRS:PATH="$MYPATH_INTEL/include" -DCFITSIO_LIBRARIES:FILEPATH="$MYPATH_INTEL/lib/libcfitsio.a;$MYPATH_INTEL/lib/liz.so" -DHEALPIX_INCLUDE_DIRS:PATH="$MYPATH_INTEL/healpix/include;$MYPATH_INTEL/healpix/include/libsharp" -DHEALPIX_LIBRARIES:FILEPATH="$MYPATH_INTEL/healpix/lib/libsharp.a;$MYPATH_INTEL/healpix/lib/libhealpix.a" -S $camb_root_dir -B $camb_root_dir/$build_dir
		#------------------------------------------------------------------------------
		# Using GNU GCC/GFortran compilers
		#------------------------------------------------------------------------------
		cmake -DCMAKE_INSTALL_PREFIX:PATH="$camb_root_dir/$build_dir/install" -DCMAKE_BUILD_TYPE=Release -DCMAKE_Fortran_COMPILER=gfortran -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DMPI_C_COMPILER=mpicc -DMPI_CXX_COMPILER=mpicxx -DMPI_Fortran_COMPILER=mpifort -DCFITSIO_INCLUDE_DIRS:PATH="$MYPATH_GNU/include" -DCFITSIO_LIBRARIES:FILEPATH="$MYPATH_GNU/lib/libcfitsio.a;$MYPATH_GNU/lib/liz.so" -DHEALPIX_INCLUDE_DIRS:PATH="$MYPATH_GNU/healpix/include;$MYPATH_GNU/healpix/include/libsharp" -DHEALPIX_LIBRARIES:FILEPATH="$MYPATH_GNU/healpix/lib/libsharp.a;$MYPATH_GNU/healpix/lib/libhealpix.a" -S $camb_root_dir -B $camb_root_dir/$build_dir
		#------------------------------------------------------------------------------
		# Build and install command
		#------------------------------------------------------------------------------
		cmake --build $camb_root_dir/$build_dir --target install -j $physicalCpuCount #-v 
